...
const Signup = () => {
  ...
  const onCreateNewUser = async (e: FormEvent<HTMLFormElement>) => {
    e.preventDefault();
    try {
      const newUser = await appwriteService.createUserAccount(formData);
      if (newUser) {
        setAuthStatus(true);
        router.push("/profile");
      }
  ...
  return (
    <div className="flex items-center justify-center">
      <div className={`mx-auto w-full max-w-lg bg-gray-200/50 rounded-xl p-10`}>
        ...
        <form onSubmit={onCreateNewUser} className="mt-8">
